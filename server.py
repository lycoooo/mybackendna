...
/api/run endpoint na gumagawa ng buong daloy — banner scan + trial signup —
na may kasamang payload headers (mobile User-Agent + Cookie nfvdid/flwssn),
eksaktong gaya ng net.py. Kaya HINDI kailangan ng CORS proxy.
...
# Sa Wasmer Edge, ang platform ang nagbibigay ng PORT env var.
# Sa local, default ay 8000 (python server.py).
HOST = "0.0.0.0"
...
async def run_all(email, nfvdid, flwssn):
    """Buong daloy gaya ng main() ng net.py: banner scan muna, saka signup.
    Babalik ng isang JSON dict na may lahat ng kailangan ng UI."""
    banner = await check_banner(nfvdid, flwssn)
    detected = bool(banner and "30" in banner.lower())
    result = await send_signup(email, nfvdid, flwssn)
    result["banner"] = banner
    result["detected"] = detected
    result["nfvdid"] = nfvdid
    result["flwssn"] = flwssn
    result["email"] = email
    return result


class Handler(SimpleHTTPRequestHandler):
    """Sinisilbihan ang static files + ang /api/run endpoint."""
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/run":
            ...
            if "@" not in email:
                self._json({"success": False, "step": 0, "error": "invalid email"},
                           status=400)
                return
            try:
                result = asyncio.run(run_all(email, nfvdid, flwssn))
                self._json(result)
            except Exception as e:  # noqa: BLE001
                self._json({"success": False, "step": 0, "error": str(e)}, status=502)
            return
        super().do_GET()
...
def main():
    ...
    print(f"\n\u2705 Netflix Trial Sender server: http://127.0.0.1:{PORT}")
    print("   (static files: index.html, style.css, script.js)")
    print("   (/api/run   — banner scan + trial signup, gaya ng net.py)")
